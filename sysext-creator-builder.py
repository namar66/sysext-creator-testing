        ...
        if os.path.exists(etc_src):
            ...
            for root, dirs, files in os.walk(etc_src):
                for f in files:
                    ...
                    # Přidáme řádek do tmpfiles.d (L+ vytvoří symlink a přepíše existující)
                    # Formát: L+ /etc/cesta - - - - /usr/lib/sysext-creator/etc-template/name/cesta
                    tmpfiles_content.append(f"L+ /etc/{rel_f_path} - - - - /{etc_template_dir}/{rel_f_path}")

            # Smažeme prázdné /etc v staging rootu
            shutil.rmtree(etc_src)

            # Zapíšeme tmpfiles.d konfiguraci
